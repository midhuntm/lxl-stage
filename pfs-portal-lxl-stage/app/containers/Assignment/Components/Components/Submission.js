...
const Submission = props => {
  ...
  const [wordLimitEnable, setwordLimitEnable] = React.useState(false);
  ...
  const onHandleSubmissionType = (event, checked) => {
    ...
    } else if (field == 'wordLimitEnable') {
      setwordLimitEnable(checked);
    }
  };
  ...
  return (
    <React.Fragment>
      <Box className={classes.container} mt={2}>
        ...
        {values.onlineText && ( // Word limit
          <Grid
            container
            item
            md={9}
            style={{ marginTop: 16, display: 'flex', alignItems: 'end' }}
          >
            <Grid sm={3} md={3}>
              <KenInputField
                placeholder=""
                ...
                name="wordLimit"
                defaultValue={() => {
                  setFieldValue('wordLimit', 0);
                }}
                onChange={newValue => {
                  setFieldValue('wordLimit', newValue.target.value);
                }}
                errors={errors?.wordLimit}
                disabled={!values.wordLimitEnable}
                touched={touched?.wordLimit}
              />
            </Grid>
            {/* <Grid md={3} item style={{ marginLeft: 10 }}>
                            <KenCheckBox
                                // disabled={values.disabled}
                                label={"Enable"}
                                value={values.wordLimitEnable}
                                name="wordLimitEnable"
                                onChange={onHandleSubmissionType}
                            />
                        </Grid> */}
          </Grid>
        ...
        {fileSubmission && (
          <React.Fragment>
            <Grid container item md={9} style={{ marginTop: 16 }}>
              <Grid sm={6} md={4}>
                <KenInputField
                  ...
                  // defaultValue={() => {
                  //   setFieldValue('noOfFilesUpload', 0);
                  // }}
                  onChange={newValue => {
                    setFieldValue('noOfFilesUpload', newValue.target.value);
                  }}
              ...
              <Grid sm={6} md={4} style={{ marginLeft: 10 }}>
                <KenSelect
                  ...
                  onChange={newValue => {
                    setFieldValue('maximumFileSize', newValue.target.value);
                  }}
            ...
            <Grid style={{ marginTop: 16 }}>
              <Typography className={classes.gradeLabel}>
                {t('labels:Submission_File_Format')}
              </Typography>
                  ...
                  return (
                    <Grid md={4} item>
                      <KenCheckBox
                        ...
                        disabled={
                          values.fileFormatLists[i].type == 'pdf' ? true : false
                        }
